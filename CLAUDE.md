...
**Fase 2 (activa desde 2026-08-29):** brief del cliente en `docs/fase2-dental-mirage.md`, plan fase por fase en `docs/implementation-plan.md` §11, decisiones de arquitectura en `docs/tradeoffs.md` TR-078 a TR-099, resumen en `docs/dental-mirage-spec.md` §11. **Brief extra post-QA (2026-09-06, 5 ítems más):** `docs/fase2.3-extra-dental-mirage.md`, plan en `docs/implementation-plan.md` §11.5, orden confirmado por el cliente 2.3.1 → 2.3.2 → 2.3.5 → 2.3.3 → 2.3.4. **2.3.1 implementado y mergeado a `dev` (TR-094, PR #5):** rediseño completo del dashboard "Turnero" — 6 tarjetas (turnos de hoy/próximos/horarios reservados con cuerpo scrolleable y deep-link real al calendario/turno exacto, total confirmados, resueltos, y una nueva "Estadística" con asistidos/ausentes acumulados). **2.3.2 implementado, en QA con el cliente (TR-095 a TR-099):** banner de conflicto en el calendario ("Tenés X turnos en conflicto, tocá para ver") — y, pedido por el cliente durante esa misma QA, tres funciones más que no estaban en el brief original: Autoreservar turnos (reprograma automáticamente turnos en conflicto al próximo horario libre, por orden de horario original, con pantalla de confirmación anterior→nuevo y textura rayada en el calendario), excepciones de horario de atención visualizadas en el calendario (mismo mecanismo de cluster/"Ver eventos →" que horarios reservados — una excepción sola es una tarjeta suave "No trabajo en este período", clickeable; mezclada con un horario reservado o un turno pasa a "Ver eventos →") y Preferencia de atención por tipo de consulta (ventana horaria opcional que acota, nunca ensancha, la disponibilidad real de ese tipo). Los ítems 2.3.3 a 2.3.5 siguen sin implementar ni rama abierta: en Turnos, motivo/mail largos pasan a "Ver motivo"/"Ver mail" + "Ver paciente" + filtro rápido HOY/SEMANA/MES; y **eliminan el estado `pendiente` de `Turno` por completo** (reemplazado por un formulario público que asigna horario real de entrada, igual que "Agregar turno" del profesional) — esto también suma unicidad de DNI por clínica en `Paciente` (hoy no existe). Calendario mobile con encabezados que acompañan el scroll (ítem 1), ajustes de horario de atención/tipos de consulta/horarios reservados generales y específicos — **sin previsualización en vivo**, descartada explícitamente por el cliente (TR-084) — con disponibilidad real calculada contra horario de atención/horarios reservados/turnos ya agendados (TR-086, reemplaza el campo de hora libre de la primera entrega), selección de horario por el paciente en el formulario público, y compartir calendario por link efímero. La vista "pantalla grande" del calendario en mobile (ítem 2) se **descartó por completo** tras fallar en dispositivo real (TR-085) — no existe en el código. Horarios reservados que se solapan (entre sí o con un turno) se agrupan en una tarjeta única "Ver eventos →" (TR-089/090, reemplaza cualquier diseño anterior de "turno rayado"); un turno en conflicto que ya pasó pasa a "resuelto" sin el aviso urgente. No se puede reservar un horario en el pasado, hay un acceso rápido "+ Reservar horario" junto a "+ Agregar turno", y un específico ya vencido se elimina directo desde el calendario (TR-091). Un turno resuelto admite marcar asistió/ausente — **irreversible**, con aviso de confirmación y rechazo del backend ante cualquier intento posterior (TR-092). Ver "Flujo de ramas" abajo — esta fase tiene un proceso de QA propio, distinto del criterio general.
