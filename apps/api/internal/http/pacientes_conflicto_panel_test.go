...
func crearTurnoVigenteDePruebaParaPaciente(t *testing.T, gdb *gorm.DB, paciente db.Paciente, tipoConsultaID string, inicio time.Time) db.Turno {
	...
	turno := db.Turno{
		...
		AtendidoPorUserID: ptrUUID(ownerDePrueba(t, gdb, paciente.ClinicID)),
...
func TestResolverConflictoPaciente_EsVerificadoMigraTutores(t *testing.T) {
	...
	turno := db.Turno{
		ClinicID:          pid,
		AtendidoPorUserID: ptrUUID(ownerDePrueba(t, gdb, pid)), PacienteID: &enConflicto.ID, Estado: "agendado", TipoConsultaID: &tid,
