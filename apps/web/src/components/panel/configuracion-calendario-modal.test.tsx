...
const { ConfiguracionCalendarioModal } =
  await import("./configuracion-calendario-modal");

const reglaGeneral = {
  id: "gen-1",
  especifico: false,
  diaSemana: 2,
  alcance: "semana" as const,
  horaDesde: "07:00",
  horaHasta: "08:00",
...
describe("ConfiguracionCalendarioModal", () => {
  ...
  it("muestra el mensaje de vacío cuando no hay reglas", async () => {
    ...
    expect(
      await screen.findByText(
        "Todavía no cargaste ningún horario reservado general.",
      ),
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        "Todavía no cargaste ningún horario reservado específico.",
      ),
    ).toBeInTheDocument();
  });
  ...
  it("guarda el horario de atención general editado", async () => {
    ...
    putHorarioAtencionGeneralActionMock.mockResolvedValue({
      horario: {
        id: "gen-h",
        alcance: "general",
        horaDesde: "09:00",
    ...
    expect(putHorarioAtencionGeneralActionMock).toHaveBeenCalledWith({
      horaDesde: "09:00",
      horaHasta: "17:00",
    });
  ...
  describe("Excepciones de horario", () => {
    ...
    function seccionExcepciones() {
      return within(
        screen.getByText("Excepciones de horario").closest("section")!,
      );
    }
    ...
    it("agrega una excepción nueva", async () => {
      ...
      await user.click(
        seccionExcepciones().getByRole("button", { name: "+ Excepción" }),
      );
      const subModal = within(
        screen.getByRole("dialog", {
          name: "Agregar excepción de horario de atención",
        }),
      );
      fireEvent.change(subModal.getByLabelText("Desde"), {
        target: { value: "09:00" },
      });
      fireEvent.change(subModal.getByLabelText("Hasta"), {
        target: { value: "13:00" },
      });
    ...
    it("marca 'No trabajo este período' y agrega una excepción sin horario", async () => {
      ...
      expect(crearHorarioAtencionActionMock).toHaveBeenCalledWith({
        alcance: "semana",
        noTrabaja: true,
      });
      expect(await screen.findByText("No trabaja")).toBeInTheDocument();
    ...
    it("elimina una excepción, pidiendo confirmación primero", async () => {
      ...
      const filaExcepcion = seccionExcepciones()
        .getByText("Esta semana")
        .closest("tr")!;
      await user.click(
        within(filaExcepcion).getByRole("button", {
          name: "Eliminar excepción de horario",
        }),
      );
      expect(eliminarHorarioAtencionActionMock).not.toHaveBeenCalled();

      await user.click(
        within(filaExcepcion).getByRole("button", { name: "Sí, eliminar" }),
      );
      expect(eliminarHorarioAtencionActionMock).toHaveBeenCalledWith("exc-1");
      await waitFor(() =>
        expect(
          seccionExcepciones().queryByText("Esta semana"),
        ).not.toBeInTheDocument(),
      );
    });
    ...
    it("'Cancelar' en la confirmación de eliminar una excepción no llama a la acción", async () => {
      ...
      await user.click(
        within(filaExcepcion).getByRole("button", { name: "Cancelar" }),
      );
  ...
  it("tocar una fila en mobile agrega una fila de acciones debajo, no la extiende hacia el costado", async () => {
    ...
    expect(within(filaGeneral.parentElement!).getAllByRole("row")).toHaveLength(
      1,
    );
    ...
    expect(
      within(filaAcciones).getByRole("button", {
        name: "Editar horario reservado",
      }),
    ).toBeInTheDocument();
    expect(within(filaAcciones).getByRole("cell")).toHaveAttribute(
      "colspan",
      "5",
    );
  });
  ...
  it("en mobile, Editar/Eliminar tienen la misma apariencia de pastilla que TurnosTable", async () => {
    ...
    const filaGeneral = screen.getByText("Martes").closest("tr")!;
    await user.click(filaGeneral);
    const filaAcciones = within(filaGeneral.parentElement!).getAllByRole(
      "row",
    )[1];
    const editar = within(filaAcciones).getByRole("button", {
      name: "Editar horario reservado",
    });
    const eliminar = within(filaAcciones).getByRole("button", {
      name: "Eliminar horario reservado",
    });
  ...
  it("elimina un horario reservado general de la tabla, pidiendo confirmación primero", async () => {
    ...
    const filaGeneral = screen.getByText("Martes").closest("tr")!;
    await user.click(
      within(filaGeneral).getByRole("button", {
        name: "Eliminar horario reservado",
      }),
    );
    ...
    await user.click(
      within(filaGeneral).getByRole("button", { name: "Sí, eliminar" }),
    );
  ...
  it("abre el sub-modal de 'Agregar horario reservado' general y agrega una fila a la tabla", async () => {
    ...
    const seccionGenerales = screen
      .getByText("Horarios reservados generales")
      .closest("section")!;
    await user.click(
      within(seccionGenerales).getByRole("button", { name: "+ Horario" }),
    );
    ...
    await waitFor(() =>
      expect(
        screen.queryByRole("dialog", {
          name: "Agregar horario reservado general",
        }),
      ).not.toBeInTheDocument(),
    );
  ...
  it("resalta y scrollea hasta la regla pedida por reglaAFocalizarId", async () => {
    render(
      <ConfiguracionCalendarioModal
        onClose={vi.fn()}
        reglaAFocalizarId="gen-1"
      />,
    );

    const fila = await screen
      .findByText("Martes")
  ...
  it("resalta y scrollea hasta la excepción pedida por horarioAtencionAFocalizarId", async () => {
    ...
    render(
      <ConfiguracionCalendarioModal
        onClose={vi.fn()}
        horarioAtencionAFocalizarId="exc-1"
      />,
    );
    ...
    const seccion = within(
      (await screen.findByText("Excepciones de horario")).closest("section")!,
    );
  ...
  describe("Tipos de consulta", () => {
    ...
    it("muestra el mensaje de vacío sin tipos de consulta", async () => {
      ...
      render(<ConfiguracionCalendarioModal onClose={vi.fn()} />);
      expect(
        await screen.findByText("Todavía no cargaste ningún tipo de consulta."),
      ).toBeInTheDocument();
    });
    ...
    it("agrega un tipo de consulta nuevo", async () => {
      ...
      const subModal = within(
        screen.getByRole("dialog", { name: "Agregar tipo de consulta" }),
      );
    ...
    it("edita un tipo de consulta existente", async () => {
      ...
      await user.click(
        screen.getByRole("button", { name: "Editar tipo de consulta" }),
      );
      const subModal = within(
        screen.getByRole("dialog", { name: "Editar tipo de consulta" }),
      );
      expect(subModal.getByLabelText("Nombre")).toHaveValue("Consulta general");
      await user.click(subModal.getByRole("button", { name: "Guardar" }));

      expect(editarTipoConsultaActionMock).toHaveBeenCalledWith(
        "tc-1",
        expect.any(Object),
      );
      expect(
        await screen.findByText("Consulta general (editada)"),
      ).toBeInTheDocument();
    });
    ...
    it("elimina un tipo de consulta, pidiendo confirmación primero", async () => {
      ...
      await user.click(
        screen.getByRole("button", { name: "Eliminar tipo de consulta" }),
      );
    ...
    it("muestra el error si no se puede eliminar (turnos asociados)", async () => {
      ...
      expect(await screen.findByRole("alert")).toHaveTextContent(
        "no se puede eliminar un tipo de consulta con turnos asociados",
      );
