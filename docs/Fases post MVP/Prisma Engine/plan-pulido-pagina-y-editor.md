...
**Estado:** los 27 hallazgos resueltos (PP-7 sin mergear) · **Fecha:** 2026-09-25 · **Alcance:** `/personalizar-pagina`, `ClinicaPublicaTemplate` y `packages/prisma-engine`. Nada del panel de gestión ni del wizard de turno (salvo el botón que lo abre).
...
| PP-3 | `feature/pp-3-historial-y-dialogos` | H2 (historial + restaurar), H10 y H19 con un diálogo propio, deshacer "Quitar" — TR-170 | PR #64, mergeado |
| PP-4 | `fix/pp-4-accesibilidad-editor` | H9, H11, H12, H14 (editor), H13 — TR-171 | PR #65, mergeado |
| PP-5 | `feature/pp-5-editor-mobile` | H15, H17 y el orden de pantallas en celular — TR-172 | PR #66, mergeado |
| PP-6 | `feature/pp-6-editor-simple` | H18, H20, H21, H23 — TR-173 | PR #67, mergeado |
| PP-7 | `fix/pp-7-hallazgos-pendientes` | H22, H25, H26, H27 (decididos con Juan el 2026-09-25), el reintento del build en CI y la hidratación de los efectos — TR-174 | implementado |

### PP-7 — lo que quedaba, decidido con Juan (2026-09-25)

- **H25** — el contenido llega a ~1100 px en escritorio (grillas de hasta 3 columnas); los textos largos siguen a ~700 px. Equipo y Horarios respetan la alineación de su sección como el resto (centrado por defecto).
- **H26** — teléfono en formato local ("351 000-0000"; el link de llamar/WhatsApp no cambia). "Llamado a la acción" nace apuntando a WhatsApp ("Escribinos"); "Pedí un turno" sigue como opción, no como default. Sin foto, la portada es una banda con el color de acento del tema. Footer simple: nombre, dirección, teléfono, redes, © año y "Hecho con PRISMA" (link a la home).
- **H27** — la portada nombra solo a la clínica, nunca a un profesional (el equipo va en el módulo Equipo).
- **H22** — una vez por día se borran las fotos que no usa el borrador ni ninguna versión publicada y tienen más de 24 h. El historial no se limita.
- **CI** — si `next build` falla (el fallo intermitente de las fuentes de Google), el paso lo reintenta una vez.
- **Al implementar, dos más con Juan:** las seis plantillas y los dos presets de estilo pasan de portada "mínima" a "centrada" (con "mínima" la banda no se veía nunca), y el Llamado a la acción de las plantillas pasa a WhatsApp sin respaldo. Y se arregla acá el desajuste de hidratación de los efectos de entrada con movimiento reducido, que venía de PP-1.
...
- **2026-09-25** — #64 mergeado a `dev`. PP-4 implementado (TR-171): `GrupoDeOpciones` (radio de WAI-ARIA) en todos los grupos del editor, `Pestanas` con `tabpanel`, tono secundario a `grafito/75` con test de contraste sobre la paleta, 44 px con `pointer: coarse`, y "Descripción de la foto" en portada (columna `foto_portada_alt`), foto suelta (`fotoAlt`) y galería (`fotoAlts`). **Primera vez que el editor se vio en un navegador real** (API + web locales, Chrome headless por CDP con una cuenta de prueba): axe sin violaciones sobre todo el documento, incluida Diseño con un tema elegido; 4 grupos → 1 parada de Tab cada uno; asa, ↑↓, pastillas y pestañas a 44 px con emulación táctil. Y dos defectos de PP-3 que solo se veían ahí: el `<header>` de `Dialogo` era un segundo `banner`, y `Dialogo` rompía el SSR — `/personalizar-pagina` daba **500** en una página nunca personalizada (la galería se abre sola).
- **2026-09-25** — #65 mergeado a `dev`. PP-5 implementado (TR-172): debajo de `lg`, Editar o Vista previa de a una con una barra fija abajo; Guardar + Publicar + "Más" en la barra de acciones; vista previa al ancho real (el selector 390/768/escritorio solo desde `lg`); sin el botón de retraer; `px-4`. Verificado en Chrome headless a 390×844 con emulación táctil contra la API local: sin scroll horizontal, axe sin violaciones en Editar, en "Más" abierto y en Vista previa, y en escritorio las dos vistas juntas como antes. De paso: el `DndContext` de la lista de módulos daba un desajuste de hidratación (ids de dnd-kit con contador global) — recibe `id` de `useId`.
- **2026-09-25** — #66 mergeado a `dev`. PP-6 implementado (TR-173): "Más opciones" cerrado por defecto en cada módulo; Diseño arranca por los presets y termina con movimiento; "+ Agregar sección" abre un catálogo con miniaturas (todo módulo tiene una) e inserta debajo del módulo abierto; tocar una sección de la vista previa abre su módulo con el foco ahí, y abrir un módulo lleva la vista previa hasta su sección (desde `lg`, dos columnas `sticky` con su propio scroll); "Google y redes", "Ocultando…/Mostrando…" y el dominio real en la vista de Google. Verificado en Chrome headless contra la API local: catálogo con 18 opciones y sus miniaturas, la sección nueva entra donde corresponde, abrir Especialidades desde la lista mueve la vista previa 231 px y **no** la ventana, tocar su sección la abre y le da el foco (también en un celular, donde vuelve a Editar), axe sin violaciones y consola limpia. **Con PP-6 se cierran los seis PRs del plan.** Siguen abiertos H22 (fotos huérfanas al descartar, sin PR asignado) y H25–H27 (decisiones de diseño con Juan o el cliente).
- **2026-09-25** — #67 mergeado a `dev`. H25–H27 y H22 decididos con Juan en la sesión y PP-7 implementado (TR-174): contenido a ~1100 px con párrafos a ~65ch; Equipo y Horarios siguen la alineación de la sección; la portada nombra solo a la clínica y, sin foto, es una banda con los colores de contraste del tema; teléfono en formato local; footer con "Hecho con PRISMA"; Llamado a la acción a WhatsApp (default y plantillas); plantillas y presets a portada "centrada"; limpieza diaria de fotos huérfanas en el backend (`internal/limpieza`, `storage.Limpiable`); reintento de `next build` en CI y en el Dockerfile; y el desajuste de hidratación de `EfectoEntrada` con movimiento reducido (28 errores por página, desde PP-1). Verificado sobre las demos en Chrome headless, escritorio y celular, con y sin movimiento reducido: 1120 px, banda visible, footer, sin desborde, axe sin violaciones, consola sin errores y ningún efecto sin terminar. **Con esto quedan resueltos los 27 hallazgos del plan.**
