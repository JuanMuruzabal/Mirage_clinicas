...
**Segunda corrección post-entrega, mismo día (2026-08-24), pedido explícito del cliente — ver TR-022 en `docs/tradeoffs.md`:** el blueprint (`render.yaml`) todavía no está aplicado en Render — no existen `RENDER_DEPLOY_HOOK_API`/`_WEB` como secrets del repo, así que el job `deploy` fallaba en rojo en cada push a `dev` sin que hubiera nada roto en el código. El job se saca por completo de `ci.yml` (no solo se retarget) hasta terminar de configurar Render; queda un comentario en su lugar explicando qué falta y el texto del job recuperable del historial de git. T5.5 sigue 🔶 (marcada) — falta aplicar el blueprint en el dashboard de Render, cargar los secrets, y recién ahí reponer el job `deploy`.
