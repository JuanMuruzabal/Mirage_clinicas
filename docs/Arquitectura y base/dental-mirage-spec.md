...
- **Los tipos de consulta (2026-09-15, TR-145).** Un tipo se identifica por su **nombre**: "Limpieza dental" es lo mismo en toda la clínica, y el color y los minutos son preferencia de la agenda de cada uno. La TR-142 había decidido que el tipo de un colega se copia, pero no qué cuenta como "el mismo tipo", y de esa pregunta sin responder salieron tres síntomas: se podía crear "Consulta general" dos veces en la propia lista, lo que ya se tenía se seguía ofreciendo como punto de partida con un cartel engañoso (*"ya tenés uno parecido"*, cuando es el mismo tipo con otro color), y el historial del paciente mostraba "—" en el turno de un colega porque resolvía el tipo contra la lista propia. Ahora el alta rechaza el duplicado con 409 —por igualdad exacta normalizada, no por el fuzzy que usa el listado: esconder de más cuesta una sugerencia, bloquear de más es una pared—, la lista de sugerencias llega ya sin lo propio, el turno viaja con el nombre y el color del tipo resueltos, y se suma un repertorio de 14 tipos odontológicos reales como **sugerencia** (no una semilla de la base: elegir uno rellena el formulario y el tipo nace propio y editable), para que la clínica recién creada no vea la sección vacía.
