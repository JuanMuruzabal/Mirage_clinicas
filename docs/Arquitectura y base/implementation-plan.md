...
| 3.2.3 | Onboarding nuevo y "¿dónde trabajás hoy?" — ✅ **completa (2026-09-13, TR-139)**. `/clinicas` como punto de partida de toda sesión · la clínica activa vive en `sessions.clinic_id` y se revalida contra la membresía · el wizard queda en un paso (perfil) y crear clínica pasa a ser opcional · `users.codigo_invitacion` (`PR-XXXX-XXXX`, 24 h) · ronda de QA: rediseño del registro y de los dos modales del alta, y el header que ofrecía `/clinicas` estando en `/clinicas` | ✅ Toda sesión arranca eligiendo clínica; entrar a una cambia lo que devuelve el panel |
